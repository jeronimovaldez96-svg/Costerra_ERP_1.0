...
describe('Return & Credit Note Reversal System', () => {
  test('Complete Execution & Restocking flow', () => {
    // 1. Setup Base Models
    const supplier = createSupplier({ name: 'Ret Supplier' })
    const product = createProduct({ name: 'Ret Widget', productGroup: 'A', productFamily: 'B', color: 'C', defaultUnitCost: 10, defaultUnitPrice: 20 })
    
    // Intake 100 widgets @ $10 each
    const po = createPurchaseOrder({ supplierId: supplier.id }, [{ productId: product.id, quantity: 100, unitCost: 10 }])
    ...
    const client = createClient({ name: 'Bob', surname: 'Returns' })
    const lead = createSalesLead(client.id, 'Lead 1')
    
    // Selling 30 items @ $20 = $600
    const quote = createQuote({ salesLeadId: lead.id }, [{ productId: product.id, quantity: 30 }])
    transitionQuote(quote.id, 'SENT')
    
    // Execute Sale
    const saleRaw = executeSale(quote.id)
    const sale = getSale(saleRaw.id)

    // Verify Inventory Post-Sale
    const sum1 = getInventorySummary()
    ...
    const ret = createReturn(sale.id, 'Customer changed mind', [
    ...
    const processed = processReturn(ret.id)
    expect(processed.status).toBe('PROCESSED')

    // 5. Verify physical restocking logic via exact Return Batches
    const sum2 = getInventorySummary()
    ...
    const fullRet = getReturnById(ret.id)
    ...
    expect(() => createReturn(sale.id, 'Trying too many', [
    ...
    expect(() => processReturn(ret.id)).toThrow(/DRAFT/)
  })
})
