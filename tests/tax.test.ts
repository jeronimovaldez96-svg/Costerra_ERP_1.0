...
describe('Tax Engine System', () => {
  test('Tax Profile Lifecycle End-to-End', async () => {
    ...
    const fetchedProfile = await getTaxProfile(globalProfileId)
    ...
    const updated = await updateTaxProfile(globalProfileId, { name: 'NY Metro Rate (Updated)' }, [
    ...
    const refetched = await getTaxProfile(globalProfileId)
    expect(refetched.components.length).toBe(2)

    // 4. Updates metadata without mutating components
    const updatedMeta = await updateTaxProfile(globalProfileId, { isActive: false })
    ...
    const list = await listTaxProfiles({ page: 1, pageSize: 10, search: 'Metro' })
    expect(list.total).toBe(1)

    const fullList = await listTaxProfiles({ page: 1, pageSize: 10 })
